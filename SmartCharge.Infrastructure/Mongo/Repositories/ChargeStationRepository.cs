...
namespace SmartCharge.Infrastructure.Mongo.Repositories
{
    internal sealed class ChargeStationRepository : IChargeStationRepository
    {
        ...
        {
            var stationsCursor = await _chargeStationDocuments.FindAsync(x => x.Id == chargeStationId.ToString()).ConfigureAwait(false);
            var stationDocument = stationsCursor.FirstOrDefault();

            if (stationDocument != null)
            {
                var groupsCursor = await _chargeGroupsDocuments.FindAsync(x => x.Id == stationDocument.ChargeGroupId).ConfigureAwait(false);
        ...
        public async Task<ChargeStation> GetAsyncExtended(Guid chargeStationId)
        {
            ...
            {
                var stationsInGroupCursor = await _chargeStationDocuments.FindAsync(x => x.ChargeGroupId == stationToReturnDocument.ChargeGroupId).ConfigureAwait(false);
                var groupsCursor = await _chargeGroupsDocuments.FindAsync(x => x.Id == stationToReturnDocument.ChargeGroupId).ConfigureAwait(false);
                var groupDocument = groupsCursor.FirstOrDefault();


                return stationToReturnDocument?.AsEntityExtended(groupDocument?.AsEntityExtended(stationsInGroupCursor.ToEnumerable().Select(x => x.AsEntityExtended(groupDocument?.AsEntity()))));
            }
        ...
        public async Task UpdateNameAsync(Guid id, string name)
        {
            await _chargeStationDocuments.FindOneAndUpdateAsync(
                    Builders<ChargeStationDocument>.Filter.Where(rec => rec.Id == id.ToString()),
        ...
        {
            await _chargeStationDocuments.FindOneAndUpdateAsync(
                    Builders<ChargeStationDocument>.Filter.Where(rec => rec.Id == chargeStation.Id.ToString()),
                    Builders<ChargeStationDocument>.Update
                    .Set(rec => rec.Connectors, chargeStation.Connectors.Select(c => c.AsDocument()))
        ...
        {
            var result = await _chargeStationDocuments.DeleteOneAsync(x => x.Id == chargeStationId.ToString()).ConfigureAwait(false);
            return result.DeletedCount;
        }

        public async Task<bool> ExistsAsync(Guid chargeStationId)
        {
            var result = await _chargeStationDocuments.FindAsync(rec => rec.Id == chargeStationId.ToString()).ConfigureAwait(false);
            return result.Any();
        }
